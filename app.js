  ...
  authPage.innerHTML = `
    <div class="auth-card">
      ...
      <div class="auth-form">
        ...
        <p class="muted small">Prototype note: any 6-digit OTP works. Admin login requires the secret code.</p>
      </div>
    </div>
  `;
...
function handleAuth() {
  ...
  if (state.role === "admin" && state.signup.adminSecret !== ADMIN_SECRET) {
...
function renderStudy() {
  ...
  document.querySelectorAll(".download-material").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      alert(`Prototype download: ${module.materialName}`);
    });
  });
...
function renderQuizOverlay() {
  ...
  study.innerHTML = `
    <div class="quiz-card">
      ...
      ${question.imageName ? `<div class="quiz-media">Image: ${question.imageName}</div>` : ""}
...
function renderAdmin() {
  ...
  document.getElementById("admin").innerHTML = `
    <div class="grid">
      <div class="panel span-5">
        ...
        <div class="school-list">
          ${state.schools.map((school) => {
            ...
            return `<button class="school-row ${state.selectedSchoolId === school.id ? "active" : ""}" data-school-id="${school.id}"><strong>${school.name}</strong><span>${school.city}, ${school.stateName} · ${count} students</span></button>`;
          }).join("")}
        </div>
        <h3>Add school</h3>
      ...
      <div class="panel span-7">
        ...
        <p class="muted">${selectedSchool.address}, ${selectedSchool.city}, ${selectedSchool.stateName} ${selectedSchool.pincode}</p>
        <div class="row">
  ...
  document.getElementById("bulkUploadBtn")?.addEventListener("click", () => alert("Bulk upload accepted for prototype validation."));
}
...
function renderSchoolTab(school, students, teachers) {
  ...
  if (state.schoolTab === "bulk") {
    return `
      ...
      <textarea id="bulkUploadText" rows="8" placeholder="For schools: name,address,city,state,pincode&#10;For students: name,email,class,school"></textarea>
      <button id="bulkUploadBtn" class="primary-button">Validate Bulk Upload</button>
    `;
  }
  return `
    <div class="table">
