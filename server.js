...
const seedData = {
  schools: [
    { id: "school-dps", name: "Delhi Public School", address: "Mathura Road", city: "Delhi", district: "New Delhi", stateName: "Delhi", pincode: "110003", region: "North", allowStudentRegistration: true, teacherAdminIds: ["tea-priya"] },
    { id: "school-greenfield", name: "Greenfield Academy", address: "Linking Road", city: "Mumbai", district: "Mumbai Suburban", stateName: "Maharashtra", pincode: "400050", region: "West", allowStudentRegistration: true, teacherAdminIds: ["tea-rohan"] },
    { id: "school-sunrise", name: "Sunrise International", address: "MG Road", city: "Bengaluru", district: "Bengaluru Urban", stateName: "Karnataka", pincode: "560001", region: "South", allowStudentRegistration: false, teacherAdminIds: [] }
  ],
  users: [
    ...
    { id: "adm-xmkc", role: "admin", name: "XMKC Admin", email: "admin@xmkc.in", schoolId: null, className: "All Classes", passwordHash: "", passwordSalt: "" }
  ],
  modules: [
...
function publicDb(db) {
  return {
    ...db,
    users: db.users.map(({ passwordHash, passwordSalt, ...user }) => ({
      ...user,
      adminPasswordSet: Boolean(passwordHash && passwordSalt)
    }))
  };
}

function mergeUsersPreservingSecrets(existingUsers, incomingUsers) {
  return incomingUsers.map((incomingUser) => {
    const existingUser = existingUsers.find((user) => user.id === incomingUser.id);
    return {
      ...incomingUser,
      passwordHash: existingUser?.passwordHash || incomingUser.passwordHash || "",
      passwordSalt: existingUser?.passwordSalt || incomingUser.passwordSalt || ""
    };
  });
}
...
function safeFileName(name) {
  ...
  const base = parsed.name.replace(/[^a-z0-9-]+/gi, "-").toLowerCase() || "upload";
  const ext = parsed.ext.replace(/[^a-z0-9.]+/gi, "").toLowerCase();
  return `${base}-${Date.now()}${ext}`;
}

function saveDataUrlUpload({ dataUrl, originalName }) {
  ensureDb();
...
async function handleApi(request, response) {
  ...
  if (request.method === "GET" && url.pathname === "/api/bootstrap") {
    sendJson(response, publicDb(readDb()));
    return true;
  }
  ...
  if (request.method === "POST" && url.pathname === "/api/state") {
    ...
    const nextDb = {
      ...
      users: payload.users ? mergeUsersPreservingSecrets(db.users, payload.users) : db.users,
  ...
  if (request.method === "POST" && url.pathname === "/api/upload") {
    const payload = await readBody(request);
    const upload = saveDataUrlUpload(payload);
    sendJson(response, { ok: true, upload });
    return true;
  }

  if (request.method === "POST" && url.pathname === "/api/admin/setup") {
    const payload = await readBody(request);
    if (payload.secret !== process.env.ADMIN_SECRET && payload.secret !== "XMKC-ADMIN-2026") {
      sendJson(response, { ok: false, error: "Invalid admin secret code" }, 403);
      return true;
    }
    if (!payload.email || !payload.password || payload.password.length < 8) {
      sendJson(response, { ok: false, error: "Email and an 8+ character password are required" }, 400);
      return true;
    }
    const db = readDb();
    let admin = db.users.find((user) => user.role === "admin" && user.email === payload.email);
    if (!admin) {
      admin = { id: `adm-${Date.now()}`, role: "admin", name: "XMKC Admin", email: payload.email, schoolId: null, className: "All Classes" };
      db.users.push(admin);
    }
    ...
    writeDb(db);
    sendJson(response, { ok: true, user: publicDb(db).users.find((user) => user.id === admin.id) });
    return true;
  }
  ...
  if (request.method === "POST" && url.pathname === "/api/admin/login") {
    const payload = await readBody(request);
    const db = readDb();
    const admin = db.users.find((user) => user.role === "admin" && user.email === payload.email);
    if (!verifyPassword(payload.password || "", admin)) {
      sendJson(response, { ok: false, error: "Invalid admin email or password" }, 401);
      return true;
    }
    sendJson(response, { ok: true, user: publicDb(db).users.find((user) => user.id === admin.id) });
    return true;
